...
function maPristup($modul="X",$castecny=false) {
	...
	if (isset($_SESSION["prava"]) && !(strpos($_SESSION["prava"], $modul)=== false)) {
		...
		$_SESSION["smlouvy"] = array ('id_smlouvy', 'typSmlouvy', 'cisloSmlouvy', 'cisloSmlouvyPom', 'datumUzavreni', 'ico', 'rodneCislo', 'smluvniStrana', 'predmet', 'cena', 'datumTxt', 'datumOd', 'datumDo', 'soubor', 'velikost', 'upozornit', 'kdy', 'text', 'uhrazeno', 'faktura', 'vazba', 'zadano', 'zadal','zmenil');
...
function povoleny($file,$dir) {
	...
	if (is_dir($dir."/".$file))
        return true;
	$pripony = array("doc","dot","xls","xlsx","docx","jpg", "gif", "tif", "txt","zip","pdf","htm","html");
