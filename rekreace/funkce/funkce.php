...
function srovnejKody($hodnota) {
	$vypis = mysql_query("SELECT kod FROM pristup WHERE kod = '$hodnota' LIMIT 1");
...
function vratIP() {
	if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	  $RIp[0] = $_SERVER["HTTP_X_FORWARDED_FOR"];
	  if ($_SERVER["HTTP_CLIENT_IP"]) {
	    $RIp[1] = $_SERVER["HTTP_CLIENT_IP"];
	  } else {
	    $RIp[1] = $_SERVER["REMOTE_ADDR"];
	  }
	} else {
	  if ($_SERVER["HTTP_CLIENT_IP"]) {
	    $RIp[0] = $_SERVER["HTTP_CLIENT_IP"];
	  } else {
	    $RIp[0] = $_SERVER["REMOTE_ADDR"];
	  }
	}
    $RIp[2]=@GetHostByAddr($RIp[0]);
    return $RIp;
}
