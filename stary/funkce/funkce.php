...
function maPristup($modul="X",$castecny=false) {
	...
	if (isset($_SESSION["prava"]) && !(strpos($_SESSION["prava"], $modul)=== false)) {
		...
		$_SESSION["opravneni"] = array ('id_jmeno', 'id_modulu', 'prava');
		...
		$_SESSION["spravce"] = "<a href=\"mailto: ulmann@scomeq.cz\">spr�vce aplikace</a>";
		return true;
	} 
	if ($castecny) 
		return false;
...
function uloz($tabulka, &$data, &$klic=0) { 
   ...
   if ($result == null) {
 	  echo "<br>Nepoda�ilo se ulo�it z�znam!!!<br>".$query."<br>";
	  return false;
   } //echo $query;
   return true;
}
